#!/usr/bin/env python3
"""Evaluate an RT-DETR checkpoint on project annotations (same protocol as DocLayout-YOLO).

Uses ``ultralytics.RTDETR`` for inference to avoid the doclayout_yolo CLI validator
incompatibility during training. Predictions are matched with greedy IoU@0.5.
"""
...
def evaluate_split(pages: list[dict], predictions: dict[str, list[dict]], split: str, threshold: float) -> dict:
    ...
    for page in pages:
        ...
        metric.update({"page_id": page["page_id"], "split": split, "method": "rtdetr", "threshold": threshold})
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=float,
        default=[0.05, 0.08, 0.1, 0.12, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.5],
    )
    ...
    (args.output_dir / "rtdetr_project_predictions.json").write_text(
    ...
    summary = {
        "model": "RT-DETR-L + project fine-tuning (ultralytics inference)",
    ...
    with (args.output_dir / "rtdetr_threshold_scan.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    (args.output_dir / "rtdetr_threshold_scan.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    (args.output_dir / "rtdetr_val_selected_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
